...
Evolução do protocolo de sockets para REST:
    DIR              -> GET  /api/files
    GET <nome>       -> GET  /api/files/{nome}
    PUT <nome> <n>   -> PUT  /api/files/{nome}   (corpo da requisição = conteúdo)

Concorrência:
  - DIR e GET são funções `def` (síncronas) -> o Starlette as executa no
    threadpool, então vários GET rodam em paralelo segurando o read-lock.
  - PUT é `async` para ler o corpo da requisição, mas delega a gravação
    bloqueante (write-lock + disco) ao threadpool via run_in_threadpool,
    evitando travar o event loop.
"""
import logging
...
async def put_file(filename: str, request: Request, storage: Storage = Depends(get_storage)):
    ...
    try:
        # Gravação bloqueante (write-lock + disco) fora do event loop.
